...
class FetchForAction:
    ...
    @staticmethod
    def set_experiment_config_handler(action_list_handler: ActionListHandler) -> None:
    ...
    def set_experiment_config_handler(exp_config_handler: ExperimentConfigHandler) -> None:
        FetchForAction.experiment_config_handler = exp_config_handler
    ...
    def post_coordinate_type() -> Tuple[str, int]:
        data = request.get_json()
        if data["marker"] == "execute_action_list":
            FetchForAction.experiment_config_handler.set_coordinate_type(
                data["type"])
            return 'Done', 201
        else:
            return 'failed', 201
